...
class FTPClient(object):
    ...
    MSG_SIZE = 1024  # Size of the message that the client can handle per single receive
    RECV_SIZE = 8192  # Maximum size of data received at once

    def __init__(self):
        """Initialize the FTP client with command line options."""
        self.username = None
        ...
        parser.add_option('-u', '--username',
                          dest='username', help='ftp username')
        parser.add_option('-p', '--password',
                          dest='password', help='ftp password')
        self.options, self.args = parser.parse_args()

        # Debugging print to verify options and arguments
    ...
    def auth(self):
        ...
        if self.options.username and self.options.password:
            ...
            count = 0
            while count < 3:  # Allow up to 3 attempts for the user to enter credentials
    ...
    def auth_user(self, username, password):
        """Send authentication data to the server and handle the response."""
        cmd = {
            'action_type': 'auth',
            'username': username,
            'password': password
        ...
        if response.get('status_code') == 201:
            create = input(
                "Incorrect username or password! Would you like to create a new user (y/n)? ")
            if create.lower() == 'y':
    ...
    def interactive(self):
        """Enter the interactive command mode after successful authentication."""
        if self.auth():
            while True:
    ...
    def check_cmd_params(self, cmd_list, min_params=None, max_params=None, exact_params=None):
        """Check if the provided command has the correct number of parameters."""
        if min_params:
    ...
    def send_msg(self, action_type, **kwargs):
        ...
        if self.MSG_SIZE > len(bytes_data):
            msg_data['fill'] = msg_data['fill'].zfill(
                self.MSG_SIZE - len(bytes_data))
            bytes_data = json.dumps(msg_data).encode()

        self.sock.send(bytes_data)

    def _ls(self, cmd_list):
        """Request a list of files from the server's current directory."""
    ...
    def _cd(self, cmd_list):
        ...
        if self.check_cmd_params(cmd_list, exact_params=1):
            ...
            if response.get('status_code') == 310:
                self.terminal_display = "[/%s%s]" % (
                    self.username, response.get('current_dir'))
            ...
            if response.get('status_code') == 301:
                ...
                with open(filename1, 'wb') as f:
                    while received_size < file_size:
                        ...
                        print('file [%s] received done! Received file size is [%s]' % (
                            filename, file_size))
            else:
                print(response.get('status_msg'))
    ...
    def create_user(self, username, password):
        """Attempt to create a new user on the server."""
        cmd = {
        ...
        if response.get('status_code') == 200:
            print("User created successfully, program exited.")
